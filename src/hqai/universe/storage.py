...
class UniverseStorage:
    ...
    def save_csv(self, dataframe: pd.DataFrame):
        ...
        log.info(f"CSV Saved -> {self.csv_file}")
    ...
    def save_parquet(self, dataframe: pd.DataFrame):
        ...
        log.info(f"Parquet Saved -> {self.parquet_file}")

    ########################################################

    def save_duckdb(self, dataframe: pd.DataFrame):

        connection = duckdb.connect(str(self.duckdb_file))

        connection.register("universe_df", dataframe)
        ...
        log.info(f"DuckDB Updated -> {self.duckdb_file}")
